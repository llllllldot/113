...
function loadMessagesFromStorage() {
    const saved = localStorage.getItem('profNavigatorMessages');
    if (saved) {
        try {
            const parsedData = JSON.parse(saved);
            if (Array.isArray(parsedData)) {
                messagesData = parsedData;
                console.log('Данные загружены:', messagesData.length, 'заявок');
            }
        } catch (e) {
...
function saveMessagesToStorage() {
    localStorage.setItem('profNavigatorMessages', JSON.stringify(messagesData));
    showSaveIndicator();
}
...
document.addEventListener('DOMContentLoaded', function() {
    loadMessagesFromStorage(); // Загружаем данные ПЕРВЫМИ
    checkAuth();
});
...
function showMainInterface() {
    ...
    setupAutoSave(); // Добавляем автосохранение
}
